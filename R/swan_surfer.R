  ...
  suppressWarnings({
    ...
    if(length(locations) == 2){
      ...
      for(i in seq_along(vals)){
        ...
        idw1_dfnar <- data.frame(idw1_spnar)[-4]# ditch option
        names(idw1_dfnar)[1] <- sparams[i]
        #idw1_dfnar <- idw1_dfnar[-(1:763),] # strip weird close to -0.1 vals
        tps_list_n_s[[i]] <- idw1_dfnar
      }
