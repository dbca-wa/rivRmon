...
When the report is read in it is tidied the following also occurs:

+ variable names are standardised
+ values are checked for "<" or ">":
  + a value following a "<" is halved
  + a value following a ">" is left as is
+ the variable emz (ecological management zone) is created based on site
...
The output from this stage will be the input for the next two stages.
...
**chloro** to integrated measurements.
...
```{r eval=FALSE}
#setup a file path to the annual report data
